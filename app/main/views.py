...
def post(id):
    ...
    page=request.args.get('page',1,type=int)
    if page == -1:
        page=(post.comments.count()-1)/current_app.config['BLOGPOLE_COMMENTS_PER_PAGE'] + 1
    pagination=post.comments.order_by(Comment.timestamp.asc()).paginate(page,per_page=current_app.config['BLOGPOLE_COMMENTS_PER_PAGE'],error_out=False)
...
def follow(username):
    ...
    if current_user.is_following(user):
        ...
        return(url_for('.user',username=username))
...
def followers(username):
    ...
    page=request.args.get('page',1,type=int)
    pagination=user.followers.paginate(page,per_page=current_app.config['BLOGPOLE_FOLLOWERS_PER_PAGE'],error_out=False)
...
def followed_by(username):
    ...
    if user is None:
        ...
        redirect(url_for('.index'))
    page=request.args.get('page',1,type=int)
    pagination=user.followed.paginate(page,per_page=current_app.config['BLOGPOLE_FOLLOWERS_PER_PAGE'],error_out=False)
...
def moderate():
    page=request.args.get('page',1,type=int)
    pagination=Comment.query.order_by(Comment.timestamp.desc()).paginate(page,per_page=current_app.config['BLOGPOLE_COMMENTS_PER_PAGE'],error_out=False)
    comments=pagination.items
